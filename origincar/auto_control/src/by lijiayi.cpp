...
// auto ob_vec=ai_msgs::msg::Target();
...
{
  {
    ...
    if (targets_msg != nullptr)
    {
      ...
      if (rec_bottom_temp > bottom_threshold_)
        found_ob = true;
...
void RacingControlNode::LineFollowing(const ai_msgs::msg::Target &target)
{
  ...
  if (mission3_flag == false)
    ...
    twist_msg.linear.x = follow_linear_speed_III;
...
{
  auto twist_msg = geometry_msgs::msg::Twist();
  float line_far_x = line_target.rois[0].rect.x_offset + line_target.rois[0].rect.width / 2;
  float angular_z1;
  float angular_z2;
  float angular_z3;
  ...
  if (avoid_flag == false)
  {
    ...
    // center_x = ob_vec.rois[0].rect.x_offset + ob_vec.rois[0].rect.width / 2;
    // float line_center= line_far.rois[0].rect.x_offset + line_far.rois[0].rect.width / 2;
    // float temp = center_x - line_center;
