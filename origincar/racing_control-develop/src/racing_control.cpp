...
bool recognize_line_left = false;
bool recognize_line_right = false;
    ...
    : rclcpp::Node(node_name, options)
{
  ...
  this->declare_parameter<float>("center_x", center_x);
  this->get_parameter<float>("center_x", center_x);
...
{
  {
    ...
    recognize_line_left = false;
    recognize_line_right = false;
    ob_flag = false;
    recognize_P = false;
    recognize_QR = false;
    QR_x = 0;
    ...
    if (targets_msg != nullptr)
    {
      for (const auto &target : targets_msg->targets)
      {
        if (target.type == "line")
        {
          recognize_line = true;
          // 识别到线在画面的左边
          if ((target.rois[0].rect.x_offset + target.rois[0].rect.width / 2) - center_x < 0)
          {
            recognize_line_left = true;
          }
          // 识别到线在画面的右边
          else
          {
            recognize_line_right = true;
          }
        }
...
void RacingControlNode::MessageProcess()
{
  ...
  while (process_stop_ == false)
  {
    ...
    {
      // if ((END_CNT >= END_CNT_threshold) && (mission3_flag == true))
      // {
      //   END_flag = true;
      // }
      if ((QRcode_flag == true) && (mission3_flag == false))
      {
      ...
      //     twist_msg.angular.z = stop_angular_vel;
      //     publisher_->publish(twist_msg);
      //     usleep(stop_tim * 1000);
      //     RCLCPP_INFO(rclcpp::get_logger("RacingControlNode"), "OverCNT_Stop!");
      //     state_flag = "Stop";
      //   }
      //   twist_msg.linear.x = 0.0;
      //   twist_msg.linear.y = 0.0;
      ...
      else
      {
        ...
        if (!targets_queue_.empty() && sub_target_ == true)
        {
          ...
          /* 执行判断任务逻辑*/
          /* 停车 */
          // if ((P_flag == true) && (Obstacle_flag == false))
          // // if ((recognize_P == true) && (ob_flag == false))
          // {
          //   // P_index记得去解除注释
          //   // const auto &P_target = targets_msg->targets[P_index];
          //   // END_flag = true;
          //   RCLCPP_INFO(rclcpp::get_logger("RacingControlNode"), "P stop!");
          //   park();
          // }
          /*任务1*/
          // else if (P_flag == false && mission3_flag == false)
...
void RacingControlNode::ObstaclesAvoiding(const ai_msgs::msg::Target &ob_target, const ai_msgs::msg::Target &line_target)
{
  ...
  while (true) // 如果巡线没有传递线的消息，进行姿态调整
  {
    ...
    if (turn_left == true)
    {
      if (recognize_line_right == true)
      {
        line_cnt += 1;
      }
    }
    // 否则看到线就跳出
    else if (recognize_line == true)
    {
      line_cnt += 1;
    }
...
//           }
//           /*任务3*/
//           else if (P_flag == false && mission3_flag == true)
//           {
//             if (Obstacle_flag == false)
//             {
//               if (Line_flag == true)
